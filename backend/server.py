...
async def export_single_visit(visit_id: str, format: str = "pdf", user=Depends(require_admin)):
    ...
    meta = [
        ...
        ["Check-In GPS", f"{v.get('check_in_lat')}, {v.get('check_in_lng')}" if v.get("check_in_lat") is not None else "—"],
        ["Check-Out", (v.get("check_out_time") or "")[:19].replace("T", " ")],
        ["Check-Out GPS", f"{v.get('check_out_lat')}, {v.get('check_out_lng')}" if v.get("check_out_lat") is not None else "—"],
